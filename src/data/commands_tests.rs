...
fn test_conn() -> rusqlite::Connection {
    ...
    crate::data::migrations::run_migrations(&conn).unwrap();
    conn
}
...
fn test_enrich_backfills_author() {
    ...
    let meta = crate::services::metadata::BookMetadata {
...
fn test_import_kindle_invalid_cover_base64_is_non_fatal() {
    ...
    let ids = import_kindle_books_db(&mut conn, &books, Some(&tmp_dir)).unwrap();
    assert_eq!(ids.len(), 1, "import should succeed despite bad base64");

    let book = get_book_db(&conn, ids[0]).unwrap();
    assert_eq!(
        book.cover_url, None,
        "cover_url should be None for bad base64"
    );
