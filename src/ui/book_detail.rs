...
pub fn BookDetail(props: BookDetailProps) -> Element {
    ...
    rsx! {
                            ...
                            for entry in diary_entries.read().iter() {
                                {
                                    ...
                                    rsx! {
                                        button {
                                            key: "{entry.id}",
                                            r#type: "button",
                                            onclick: move |_| editing_diary_entry.set(Some(entry_clone.clone())),
                                            class: "w-full text-left rounded-lg border border-gray-200 bg-gray-50 px-3 py-2
        ...
        if let Some(ref diary_entry) = *editing_diary_entry.read() {
            DiaryEntryForm {
                book_id: book_id,
                book_title: Some(bk.title.clone()),
                entry: Some(diary_entry.clone()),
                on_save: {
                    let db = db.clone();
                    move |_| {
                ...
                on_close: move |_| editing_diary_entry.set(None),
            }
        } else if *show_diary_form.read() {
            DiaryEntryForm {
                book_id: book_id,
                book_title: Some(bk.title.clone()),
                on_save: {
